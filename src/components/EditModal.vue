...
watch(() => props.show, (v) => {
  if (v) {
    name.value = props.account?.account_name || ''
    error.value = ''
    loading.value = false
  }
})
...
async function submit() {
  ...
  loading.value = true
  error.value = ''
  const result = await props.handleSubmit(name.value.trim())
    ...
    <Transition name="modal">
      ...
      >
        <div class="modal-box bg-white dark:bg-slate-900 rounded-3xl w-full max-w-sm shadow-2xl p-6">
          <h2 class="text-lg font-bold text-slate-900 dark:text-white mb-5 tracking-tight">编辑账号</h2>
